...
static void process_image(unsigned char *p, int len)
{
	...
	if (v4lconvert_convert(v4lconvert_data,
                ...
                //if (errno != EAGAIN) errno_exit("v4l_convert");
                printf("Could not convert, using default format for image\n");
		//return;
	}else{
	    p = dst_buf;
	...
	if (grab) {
		...
		f = fopen("image.ppm", "w");
                fprintf(f, "P6\n%d %d 255\n", 
                        fmt.fmt.pix.width, fmt.fmt.pix.height);
		fwrite(p, fmt.fmt.pix.sizeimage, 1, f);
		fclose(f);
		printf("image dumped to 'image.ppm'\n");
		exit(EXIT_SUCCESS);
	}
#ifdef WITH_GTK
